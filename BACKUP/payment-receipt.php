...
if (!isset($_GET["payment_no"])) {
    die("Invalid request! Payment number is missing.");
}

$payment_no = intval($_GET["payment_no"]);

// Fetch payment details
$stmt = $conn->prepare("SELECT * FROM tbl_payment WHERE payment_no = ?");
$stmt->bind_param("i", $payment_no);
$stmt->execute();
$result = $stmt->get_result();

if ($result->num_rows === 0) {
    die("Payment record not found for Payment No: $payment_no");
}
...
if ($student_result->num_rows === 0) {
    die("Student record not found for Student ID: " . $payment["student_id"]);
}
            ...
            <div class="row mt-3">
                <div class="col-md-6">
                    ...
                    <div class="mb-2 d-flex align-items-center">
                        ...
                        <input type="text" class="input-line" value="<?= htmlspecialchars($student["contact_number"] ?? 'N/A') ?>" readonly>
                    </div>
                </div>
                <div class="col-md-6">
                    <div class="mb-2 d-flex align-items-center">
                    ...
                    <div class="mb-2 d-flex align-items-center">
                        ...
                        <input type="text" class="input-line" value="<?= htmlspecialchars($payment["date_created"]) ?>" readonly>
                    </div>
                    <div class="mb-2 d-flex align-items-center">
                    ...
                    <div class="mb-2 d-flex align-items-center">
                        ...
                        <input type="text" class="input-line" value="<?= htmlspecialchars($payment["transaction_no"] ?? 'N/A') ?>" readonly>
                    </div>
                </div>
            </div>
